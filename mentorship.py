...
def gmail_send(config:dict, to_addr, msg: MIMEMultipart):
    server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
    server.ehlo()
    server.login(config['gmail_user'], config['gmail_pass'])
    server.sendmail('WiNLP Chairs', [to_addr, config['cc_user']], msg.as_string())
    server.close()
    return server
...
def generate_email(config: dict,
    ...
    for i, row in enumerate(worksheet.get_rows()):
        ...
        if sent or not award_amt.strip() or award_amt == 0:
            continue
...
-- WiNLP Chairs'''.format(name=recipient_name), recipient_email)
        ...
        if 'Moses' in recipient_name:
            gmail_send(config, recipient_email, msg)


def draft_msg(config, text, to_addr):
    """
    ...
    """
    msg = MIMEMultipart()
    msg['From'] = config['cc_user']
    msg['To'] = to_addr
    msg['CC'] = config['cc_user']
...
if __name__ == '__main__':
    ...
    p.add_argument('-s', '--spreadsheet', help='Spreadsheet with submissions, authors, etc', required=True, default='winlp_budget.xlsx')
    p.add_argument('-d', '--docx', help='Path to docx template', default='WiNLP Travel Grant Letter.docx')
    p.add_argument('-e', '--email', help='Path to template for email body.', default='grant_notification.txt')
