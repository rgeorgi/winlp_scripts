...
base_url = 'https://www.softconf.com/acl2019/WiNLP2019/'

login_url = base_url + 'login/scmd.cgi'
username = "rgeorgi"
password = "hvVWm;tcukdjdbjRY3sHtdcPMfwPrenC"
...
def author_information(session: requests.Session,
    ...
    :return:
    """
    response = session.post(url=base_url + "manager/scmd.cgi",
...
def submission_information(session: requests.Session,
                           bytes = False) -> Union[pandas.DataFrame, bytes]:
    response = session.post(
        url=base_url+'manager/scmd.cgi',
        params={'scmd':'makeSpreadsheet'},
        data={
            "Type": "submissions",
            "Field{0}": "paperID",
...
def retrieve_pdf(session: requests.Session, submission_id: int) -> bytes:
    """
    Retrieve the PDF for the given submission ID.

    :param submission_id: The int of the submission ID
    """
    response = session.get(url=base_url+'pub/scmd.cgi',
...
def download_submission_page(session: requests.Session, url):
    response = session.get(base_url + 'manager/scmd.cgi?scmd=submitPaperCustom_editor&page_theid=1')
    return response.text

# session = login(username, password)
# page_html = download_submission_page(session, None)
# with open('sub_page.html', 'w') as page_f:
#     page_f.write(page_html)
...
with open('sub_page.html', 'r') as page_f:
    convert_submission_page_to_text(page_f.read())

# if False:
#     session = login('rgeorgi',
#                     'hvVWm;tcukdjdbjRY3sHtdcPMfwPrenC',
#                     )
#     author_info = author_information(session)
#     with open('author_info.xlsx', 'wb') as author_f:
#         pickle.dump(author_info, author_f)
# else:
#     with open('author_info.xlsx', 'rb') as author_f:
#         author_info = pickle.load(author_f) # type: pandas.DataFrame
#
# print(list(author_info.keys()))
# print(author_info['RegionInfo'])


# print(author_information(session))
# print(submission_information(session))
# print(retrieve_pdf(session, 102))
